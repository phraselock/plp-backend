...
#
# Usage:
#   curl -sSL https://raw.githubusercontent.com/phraselock/plp-backend/main/install.sh | bash
#
set -euo pipefail
...
# Also re-exec when piped straight into bash ("curl | bash", no
# script-file argument): bash reads this entire script from stdin as it
# executes, which then keeps competing with whiptail for that same stdin
# further down, breaking keyboard input in its dialogs (Tab to move
# between fields/buttons especially — a real case we hit). Both cases
# share one re-exec so a non-root user on a pipe doesn't trigger it twice.
if [[ "$NEED_SUDO" == true || ! -t 0 ]]; then
  TMP_SELF=$(mktemp)
  curl -fsSL "https://raw.githubusercontent.com/phraselock/plp-backend/main/install.sh" -o "$TMP_SELF"
  chmod +x "$TMP_SELF"
  ...
  # guard would keep re-triggering on every re-exec, looping forever.
  # Safe to redirect now because bash reads the script from the file
  # argument this time, not from stdin — freeing stdin up for the
  # terminal (and for sudo's own password prompt, which wants one too).
